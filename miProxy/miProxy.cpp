...
MiProxy::MiProxy(int port, string www_ip, double alpha, char* log_file_p, int opt):
  socket_(-1), port_(port), www_ip_(www_ip), alpha_(alpha), log_file_p_(log_file_p), opt_(opt) {}

MiProxy::MiProxy(int port, string dns_ip, int dns_port, double alpha, char* log_file_p, int opt):
  socket_(-1), port_(port), dns_ip_(dns_ip), dns_port_(dns_port), alpha_(alpha), log_file_p_(log_file_p), opt_(opt) {}
...
void MiProxy::Start() {
  ofstream log_file (log_file_p_, ios::trunc);
  log_file_ = &log_file;
    ...
    for (auto &client_sock : client_sockets) {
      if (client_sock != 0 && FD_ISSET(client_sock, &readfds)) {
        ...
        if (valread == 0) {
          ...
          close(client_ip_video_socket[client_ip]);
          client_ip_video_socket.erase(client_ip);
          client_sock = 0;
        }
        else {
          buffer[valread] = '\0';
          ...
          if (nolist_request_s.size() != 0) {
            int video_server_socket = GetVideoServerSocket_(client_ip);
          ...
          int seg_pos = modded_request.find("Seg");
          ...
          if (seg_pos != string::npos) {
            ...
            double received = double(content_len + header_len)/1000;
            double T_new = double(content_len + header_len)*8/1000/diff_time;
            ...
            if (log_file_->is_open()) {
              cout << "-------------------------logfile is open-----------------" << endl;
              int seg_end_pos = modded_request.find(" ", seg_pos);
              int get_pos = modded_request.find("GET ");
              *log_file_ << client_ip << " " << modded_request.substr(get_pos + 5, seg_end_pos - get_pos - 5) << " ";
              cout << client_ip_video_server_ip[client_ip] << endl;
              *log_file_ << client_ip_video_server_ip[client_ip] << " ";
              *log_file_ << diff_time << " ";
              *log_file_ << T_new << " ";
              *log_file_ << client_ip_avg_throughput[client_ip] << " ";
              int vod_pos = modded_request.find("/vod/");
              *log_file_ << request.substr(vod_pos+5, seg_pos - vod_pos - 5) << endl;;
            }
          }
...
int MiProxy::GetVideoServerSocket_(string client_ip) {
  if (client_ip_video_socket.find(client_ip) == client_ip_video_socket.end()) {
    string video_server_ip;
    if (opt_ == 0) {
      client_ip_video_server_ip[client_ip] = www_ip_;
      video_server_ip = www_ip_;
    }
    else {
      cout << "hi" << endl;
      video_server_ip = QueryDNS(dns_ip_, dns_port_);
      client_ip_video_server_ip[client_ip] = video_server_ip;
    ...
    client_ip_video_socket[client_ip] = cur_socket;
    return cur_socket;
  }
  else {
    return client_ip_video_socket[client_ip];
  }
}
